...
// keep track of polling interval so we can turn it off when using range mode
const pollRef = useRef(null);

// -------- live data fetching (/protodata) --------
...
const raw = await getProtoData(); // calls /protodata
...
// keep only last WINDOW_DAYS of data
const now = Date.now();
...
// optional extra guard: cap number of points if it ever gets huge
const MAX_POINTS = 500;
if (cleaned.length > MAX_POINTS) {
cleaned = cleaned.slice(cleaned.length - MAX_POINTS);
}
...
const id = setInterval(() => loadLiveData(false), 60_000);
pollRef.current = id;
...
// stop live polling while range mode is active
if (pollRef.current) {
clearInterval(pollRef.current);
...
// show date+time since we’re looking at a month / range
...
const subtitle =
mode === 'live'
? `Source: /protodata • Units: m/s² (RMS) • showing last ${WINDOW_DAYS} days • auto-refresh 60s`
...
>
{loading && mode === 'range' ? 'Loading…' : 'Load Range'}
</button>

{mode === 'range' && (
<button
onClick={handleBackToLive}
className="px-4 py-2 rounded-full bg-gray-700 text-gray-100 font-semibold text-sm"
>
Back to live view
</button>
)}
</div>
{mode === 'range' && points.length > 0 && (
<p className="text-xs text-gray-400 mt-2">
Showing {points.length} points from{' '}
{formatTime(points[0].timestamp)} to{' '}
...
{mode === 'live'
? `No vibration data found for the last ${WINDOW_DAYS} days.`
...
<h2 className="text-lg font-medium text-white mb-3">
RMS Vibration History ({mode === 'live' ? `Last ${WINDOW_DAYS} Days` : 'Custom Range'})
</h2>
